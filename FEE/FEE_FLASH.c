...
void Flash_Erase(uint32_t addr)
{
    ...
    FLASH->CR1 |= FLASH_CR_SER; //Page Erase Set
    FLASH->ACR = addr; //Page Address
    FLASH->CR1 |= FLASH_CRCCR_START_CRC; //Start Page Erase
    while((FLASH->SR1&FLASH_SR_BSY));
    FLASH->CR1 &= ~FLASH_SR_BSY;
    FLASH->CR1 &= ~FLASH_CR_SER; //Page Erase Clear
}

void Flash_Write_Int(uint32_t addr, int data)
{
    Flash_Unlock();
    FLASH->CR1 |= FLASH_CR_PG;				/*!< Programming */
    while((FLASH->SR1&FLASH_SR_BSY));
